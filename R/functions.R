...
#' @param tab A table of judgement.
...
perreault_leigh_test <- \(tab) {
  ...
  n_cat <- nrow(ff_hat)
  s_hat <- sqrt((tr(ff_hat) - 1 / n_cat) / (1 - 1 / n_cat))
...
perreault_leigh_test_2 <- \(tab) {
  ...
  ff_mod_est <- \(p) {
    ...
    t <- p[3:(n_cat + 1)]
    t <- c(t, 1 - sum(t))
    middle <- (1 - s1) * (1 - s2) * one_mat / n_cat^2
    right <- s1 * (1 - s2) * tcrossprod(t, ones) / n_cat +
      s2 * (1 - s1) * tcrossprod(ones, t) / n_cat
    left <- s1 * s2 * diag(t)
    left + right + middle
  }
  f <- \(p) {
  ...
  ui <- rbind(
    ...
    c(0, 0, rep(1, n_cat - 1)),
    c(0, 0, -rep(1, n_cat - 1)),
    cbind(rep(0, n_cat - 1), rep(0, n_cat - 1), diag(n_cat - 1))
  )

  ci <- c(0, -1, 0, -1, 0, -1, rep(0, n_cat - 1))

  result <- constrOptim(
    theta = c(0.5, 0.5, rep(1 / n_cat, n_cat - 1)),
    f = f,
    grad = NULL,
    ui = ui,
    ci = ci
...
#' Test validity of the marginal guessing distribution model (1)
#'
#' @param tab A table of judgement.
#' @return A vector containing the statistic and associated p-value obtained
#'   from the chi squared test.
mgdm_test <- \(tab) {
  ...
  prod <- tcrossprod(f_hat, f_hat)
  s_2_hat <- (tr(ff_hat) - tr(prod)) / (1 - tr(prod))

  kk <- (1 - s_2_hat) * tcrossprod(f_hat, f_hat)
  ff_mod <- diag(pmax(diag(ff_hat - kk), 0)) + kk
  stat <- n * sum((ff_hat - ff_mod)^2 / ff_mod)
  c(
    stat = stat,
    p_value = pchisq(stat, n_cat^2 - n_cat, lower.tail = FALSE)
  )
}

#' Test validity of the marginal guessing distribution model (2)
#'
#' @param tab A table of judgement.
#' @return A vector containing the statistic and associated p-value obtained
#'   from the chi squared test.

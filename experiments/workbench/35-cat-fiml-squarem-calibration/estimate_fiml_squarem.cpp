...
EmIterStatus run_em_iterations(
  ...
  // One EM sweep: out = EM(cur). Both over the active set.
  auto em_step = [&](const Eigen::VectorXd& cur, Eigen::VectorXd& out) {
    out.setZero();
    for (std::size_t g = 0; g < in.group_n_subjects.size(); ++g) {
      const std::uint32_t n_comps = in.group_n_completions[g];
      if (n_comps == 0) continue;
      ...
      double sum_w = 0.0;
      for (std::uint32_t k = 0; k < n_comps; ++k) {
        wbuf[k] = cur(static_cast<Eigen::Index>(idx[k]));
        sum_w += wbuf[k];
      }
      if (sum_w <= singular_tol) continue;
      const double scale = static_cast<double>(in.group_n_subjects[g]) / sum_w;
      for (std::uint32_t k = 0; k < n_comps; ++k) {
        out(static_cast<Eigen::Index>(idx[k])) += scale * wbuf[k];
      }
    }
    out = (out.array() + delta).matrix() / (n_subj + flatten);
  };

  // SQUAREM (Varadhan & Roland 2008, scheme S3): each cycle does two EM
  // sweeps, extrapolates, then one stabilising EM sweep. Fixed points are
  // exactly the EM fixed points; the safeguard falls back to the plain EM
  // step whenever the extrapolated point leaves the simplex face.
  Eigen::VectorXd t1(theta.size()), t2(theta.size()), tx(theta.size());
  bool first_cycle = true;
  while (status.iterations < iter_cap) {
    em_step(theta, t1);
    ++status.iterations;
    double max_change = (t1 - theta).cwiseAbs().maxCoeff();
    if (first_cycle && any_inactive) {
    ...
    if (status.iterations >= iter_cap) { theta.swap(t1); return status; }

    em_step(t1, t2);
    ++status.iterations;
    if ((t2 - t1).cwiseAbs().maxCoeff() < tol) {
      theta.swap(t2);
      status.converged = true;
      return status;
    }
    ...
    if (alpha > -1.0) alpha = -1.0;  // never shorter than a plain EM step

    tx = theta - 2.0 * alpha * r + (alpha * alpha) * v;
    bool valid = true;
    for (Eigen::Index i = 0; i < tx.size(); ++i) {
      if (!(tx(i) >= 0.0) || !std::isfinite(tx(i))) { valid = false; break; }
    }
    if (valid) {
      const double sum_active = tx.sum();
      const double target = 1.0
          - inactive_next * (static_cast<double>(in.n_total_patterns)
                             - static_cast<double>(in.n_active));
      if (sum_active > zero_tol && target > 0.0) {
        tx *= target / sum_active;
      } else {
        valid = false;
      }
    }
    if (!valid) {
      theta.swap(t2);  // safeguard: keep the two plain EM steps
      continue;
    }
    em_step(tx, t1);  // stabilising sweep keeps the iteration inside EM-land
    ++status.iterations;
    theta.swap(t1);
  }
  return status;

...
| 31 flattening calibration | `experiments/studies/31-flattening-calibration/` | paper-facing study | cat_fiml flatten constant calibration | kappa-missing / library | current | ignore local CSVs | 1000-rep grid (3 latent-class DGPs C=3–5, pair-MCAR + anchor-MAR, n 20–100, c ∈ {0,.01,.1,.5,1}): c ≤ 0.1 is RMSE-free (max +0.5%) with bias shift ≤ 0.003 at n=20 → recommend flatten = 0.1 (package default stays 0 for exact-ML back-compat); c ≥ 0.5 shows visible attenuation. Flattening makes SEs conservative (se/sd 0.96 → 1.47, coverage 0.88 → 0.97) by retaining the full data-supported table (more eigenvalues just above the rcond cut); residual undercoverage at n=20, C ≥ 4 is FIML small-n point bias, not variance. Also fixed the flatten EM iteration cap (face-tolerance stopping rule). |
